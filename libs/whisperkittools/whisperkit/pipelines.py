...
class WhisperKit(WhisperPipeline):
    ...
    def build_cli(self):
        ...
        # Use subprocess.run with DEVNULL to suppress build output
        build_process = subprocess.run(
            f"swift build -c release --product {self.product_name}",
            cwd=self.repo_dir,
            shell=True,
            check=True, # Ensure it raises CalledProcessError on failure
            stdout=subprocess.DEVNULL,
            stderr=subprocess.DEVNULL
        )

        # Original code to find the binary path (no output to suppress here)
        ...
        # Log success internally, but it won't print during the build itself
        # logger.info(f"Successfuly built {self.product_name} CLI (output suppressed)")
